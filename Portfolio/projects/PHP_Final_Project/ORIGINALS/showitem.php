...
$get_item_res = mssql_query($get_item,$mssql) or die("Couldn't connect");

if (mssql_num_rows($get_item_res) < 1) {
   //invalid item
   $display_block .= "<p><em>Invalid item selection.</em></p>";
   //valid item, get info
   while ($item_info = mssql_fetch_array($get_item_res)) {
   ...
   <a href=\"seestor.php?cat_id=".$cat_id."\">".$cat_title."</a> &gt; ".$item_title."</strong></p>
   ...
   //free result
   mssql_free_result($get_item_res);

   //get colors
   $get_colors_sql = "SELECT item_color FROM store_item_color WHERE item_id = '".$_GET["item_id"]."' ORDER BY item_color";
   $get_colors_res = mssql_query($get_colors, $mssql) or die("Couldn't connect");

   if (mssql_num_rows($get_colors_res) == 0) {
        $display_block .= "<p><strong>Available Colors:</strong><br/>
        <select name=\"sel_item_color\">";

        while ($colors = mssql_fetch_array($get_colors_res)) {
   ...
   //free result
   mssql_free_result($get_colors_res);

   //get sizes
   $get_sizes_sql = "SELECT item_size FROM store_item_size WHERE item_id = ".$_GET["item_id"]." ORDER BY item_size";
   $get_sizes_res = mssql_query($get_sizes_sql, $mssql) or die("Couldn't connect");

   if (mssql_num_rows($get_sizes_res) > 0) {
       $display_block .= "<p><strong>Available Sizes:</strong><br/>
       <select name=\"sel_item_size\">";

       while ($sizes = mssql_fetch_array($get_sizes_res)) {
   ...
   //free result
   mssql_free_result($get_sizes_res);
